...
namespace MakeShpFile
{
    public partial class Form1 : Form
    {
        ...
        private void Default_Polyline_Routes(IFeatureWorkspace pFWS, string shpName, string InputFilePath, string DataStructureFile)
        {
            //开始添加属性字段；
            ...
            while ((Routes_Line = SR_Routescsv.ReadLine()) != null)
            {
                ...
                if (SumOfpoint > 11000)
                {
                    ...
                    Make_Polyline_Feature2(pFeatureBuffer, pFeatureCursor, subAryLine);
                }
                else
                {
                    string[] strArray = new string[1];
                    strArray[0] = SumPath;
                    Make_Polyline_Feature2(pFeatureBuffer, pFeatureCursor, strArray);
                }
        ...
        private void Car_Polyline_Routes(IFeatureWorkspace pFWS, string shpName, string InputFilePath,string DataStructureFile)
        {
            IField nameField = new FieldClass();
            IFieldEdit nameFieldEdit = (IFieldEdit)nameField;
            ...
            fieldChecker.ValidateWorkspace = (IWorkspace)pFWS;
            fieldChecker.Validate(fields, out enumFieldError, out validatedFields);
            IFeatureClass pNewFeaCls = null; ;
            //在工作空间中生成FeatureClass;
            try
            {
                 pNewFeaCls = pFWS.CreateFeatureClass(shpName, validatedFields, null, null, esriFeatureType.esriFTSimple, "Shape", "");
            }
            catch (System.Exception ex)
            {
        ...
        private void Riding_Polyline_Routes(IFeatureWorkspace pFWS, string shpName, string InputFilePath)
        {
            //开始添加属性字段；
            IFields fields = new FieldsClass();
            IFieldsEdit fieldsEdit = (IFieldsEdit)fields;
            ...
            while ((Routes_Line = SR_Routescsv.ReadLine()) != null)
            {
                ...
                int SumOfpoint = temp.Length - temp.Replace(";", "").Length;
                pFeatureBuffer.set_Value(11, SumOfpoint.ToString());
        ...
        private void Walking_Polyline_Routes(IFeatureWorkspace pFWS, string shpName, string InputFilePath)
        {
            //开始添加属性字段；
            IFields fields = new FieldsClass();
            IFieldsEdit fieldsEdit = (IFieldsEdit)fields;
            ...
            while ((Routes_Line = SR_Routescsv.ReadLine()) != null)
            {
                ...
                string temp = aryLineLast;
                int SumOfpoint = temp.Length - temp.Replace(";", "").Length;
                pFeatureBuffer.set_Value(10, SumOfpoint.ToString());
        ...
        private void Car_Polyline_subPaths(IFeatureWorkspace pFWS, string shpName, string InputFilePath, string DataStructureFile)
        {
            //开始添加属性字段；
            IFields fields = new FieldsClass();
            IFieldsEdit fieldsEdit = (IFieldsEdit)fields;
...
            IFeatureClass pFeatureClass = pNewFeaCls;
        ...
        private void CsvToShp(string csvFilePath)
        {
            if (DataMode == 1)//polyline
            {
                if (VehicleMode == 1) // car
                {
                    ...
                    Car_Polyline_Routes(pFWS, shpName, InputFilePath, DataStructureFile);
                ...
                else if(VehicleMode == 3)
                {
                    ...
                    Riding_Polyline_Routes(pFWS, shpName, InputFilePath);
                }
        ...
        private void ReadDb_menu_Click(object sender, EventArgs e)
        {

        }

        private void DelFeature_byAttritute_Click(object sender, EventArgs e)
        {
            IFeatureLayer pLayer = AxmapCtrl.get_Layer(0) as IFeatureLayer;
            ...
            int deleteCount = 0;
            int count =  pLayer.FeatureClass.FeatureCount(null);
            for (int i = 0; i < pLayer.FeatureClass.FeatureCount(null); i++)
            {
                clsFldValue = feature.get_Value(3);
                if (double.Parse(clsFldValue.ToString()) == 0)
                {
                    feature.Delete();
                    deleteCount++;
                }
            ...
            MessageBox.Show("共" + count.ToString() + "个要素，" + "删除" + deleteCount.ToString() + "个要素");
        }

        private void AddShpFile_Click(object sender, EventArgs e)
        {
            ...
            // 获取当前路径和文件名 
             ...
             AxmapCtrl.Map.AddLayer(pFeatureLayer);
             AxmapCtrl.ActiveView.Refresh();
        }
